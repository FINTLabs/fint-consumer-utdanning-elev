package no.novari.fint.consumer.models.varsel;
...
public class VarselController {
    ...
    public VarselResources getVarsel(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getVarselByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Varsel cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public VarselResources getVarselByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getVarsel(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private VarselResources getVarselByOdataFilter(
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new VarselResources();
    
        ArrayList<VarselResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<VarselResource>>() {});
