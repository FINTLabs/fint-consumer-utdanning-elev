package no.novari.fint.consumer.models.elevtilrettelegging;
...
import no.novari.fint.model.resource.utdanning.elev.ElevtilretteleggingResources;
import no.novari.fint.model.utdanning.elev.ElevActions;

@Slf4j
@Api(tags = {"Elevtilrettelegging"})
@CrossOrigin
@RestController
@RequestMapping(name = "Elevtilrettelegging", value = RestEndpoints.ELEVTILRETTELEGGING, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class ElevtilretteleggingController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private ElevtilretteleggingCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Elevtilrettelegging cache is disabled.");
        }
    ...
    @GetMapping
    public ElevtilretteleggingResources getElevtilrettelegging(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getElevtilretteleggingByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Elevtilrettelegging cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, ElevActions.GET_ALL_ELEVTILRETTELEGGING, client);
        ...
        Stream<ElevtilretteleggingResource> resources;
    ...
    @PostMapping("/$query")
    public ElevtilretteleggingResources getElevtilretteleggingByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getElevtilrettelegging(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ElevtilretteleggingResources getElevtilretteleggingByOdataFilter(
        ...
        Event event = new Event(
                ...
                ElevActions.GET_ELEVTILRETTELEGGING, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ElevtilretteleggingResources();
    
        ArrayList<ElevtilretteleggingResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ElevtilretteleggingResource>>() {});
    ...
    public ElevtilretteleggingResource getElevtilretteleggingBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, ElevActions.GET_ELEVTILRETTELEGGING, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<ElevtilretteleggingResource> elevtilrettelegging = cacheService.getElevtilretteleggingBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return elevtilrettelegging.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            ElevtilretteleggingResource elevtilrettelegging = objectMapper.convertValue(response.getData().get(0), ElevtilretteleggingResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(elevtilrettelegging);
        }    
    }
