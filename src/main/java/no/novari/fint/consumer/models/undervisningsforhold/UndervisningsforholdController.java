package no.novari.fint.consumer.models.undervisningsforhold;
...
import no.novari.fint.model.resource.utdanning.elev.UndervisningsforholdResources;
import no.novari.fint.model.utdanning.elev.ElevActions;

@Slf4j
@Api(tags = {"Undervisningsforhold"})
@CrossOrigin
@RestController
@RequestMapping(name = "Undervisningsforhold", value = RestEndpoints.UNDERVISNINGSFORHOLD, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class UndervisningsforholdController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Undervisningsforhold cache is disabled.");
        }
    ...
    public UndervisningsforholdResources getUndervisningsforhold(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getUndervisningsforholdByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Undervisningsforhold cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<UndervisningsforholdResource> resources;
    ...
    @PostMapping("/$query")
    public UndervisningsforholdResources getUndervisningsforholdByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getUndervisningsforhold(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private UndervisningsforholdResources getUndervisningsforholdByOdataFilter(
        ...
        Event event = new Event(
                ...
                ElevActions.GET_UNDERVISNINGSFORHOLD, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new UndervisningsforholdResources();
    
        ArrayList<UndervisningsforholdResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<UndervisningsforholdResource>>() {});
    ...
    public UndervisningsforholdResource getUndervisningsforholdBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<UndervisningsforholdResource> undervisningsforhold = cacheService.getUndervisningsforholdBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return undervisningsforhold.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            UndervisningsforholdResource undervisningsforhold = objectMapper.convertValue(response.getData().get(0), UndervisningsforholdResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(undervisningsforhold);
        }    
    }
