package no.novari.fint.consumer.models.elev;
...
import no.novari.fint.model.resource.utdanning.elev.ElevResources;
...
public class ElevController {
    ...
    public ElevResources getElev(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getElevByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Elev cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public ElevResources getElevByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getElev(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ElevResources getElevByOdataFilter(
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ElevResources();
    
        ArrayList<ElevResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ElevResource>>() {});
