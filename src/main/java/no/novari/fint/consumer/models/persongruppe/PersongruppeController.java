package no.novari.fint.consumer.models.persongruppe;
...
@Slf4j
@Api(tags = {"Persongruppe"})
@CrossOrigin
@RestController
@RequestMapping(name = "Persongruppe", value = RestEndpoints.PERSONGRUPPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class PersongruppeController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private PersongruppeCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Persongruppe cache is disabled.");
        }
    ...
    @GetMapping
    public PersongruppeResources getPersongruppe(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getPersongruppeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Persongruppe cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, ElevActions.GET_ALL_PERSONGRUPPE, client);
        ...
        Stream<PersongruppeResource> resources;
    ...
    @PostMapping("/$query")
    public PersongruppeResources getPersongruppeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getPersongruppe(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private PersongruppeResources getPersongruppeByOdataFilter(
        ...
        Event event = new Event(
                ...
                ElevActions.GET_PERSONGRUPPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new PersongruppeResources();
    
        ArrayList<PersongruppeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<PersongruppeResource>>() {});
    ...
    public PersongruppeResource getPersongruppeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, ElevActions.GET_PERSONGRUPPE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<PersongruppeResource> persongruppe = cacheService.getPersongruppeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return persongruppe.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            PersongruppeResource persongruppe = objectMapper.convertValue(response.getData().get(0), PersongruppeResource.class);
