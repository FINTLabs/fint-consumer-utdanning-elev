...
@CrossOrigin
@RestController
@RequestMapping(name = "Kontaktlarergruppe", value = RestEndpoints.KONTAKTLARERGRUPPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class KontaktlarergruppeController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Kontaktlarergruppe cache is disabled.");
        }
    ...
    @GetMapping
    public KontaktlarergruppeResources getKontaktlarergruppe(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getKontaktlarergruppeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Kontaktlarergruppe cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<KontaktlarergruppeResource> resources;
    ...
    @PostMapping("/$query")
    public KontaktlarergruppeResources getKontaktlarergruppeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getKontaktlarergruppe(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private KontaktlarergruppeResources getKontaktlarergruppeByOdataFilter(
        ...
        Event event = new Event(
                ...
                ElevActions.GET_KONTAKTLARERGRUPPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new KontaktlarergruppeResources();
    
        ArrayList<KontaktlarergruppeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<KontaktlarergruppeResource>>() {});
    ...
    public KontaktlarergruppeResource getKontaktlarergruppeBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<KontaktlarergruppeResource> kontaktlarergruppe = cacheService.getKontaktlarergruppeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return kontaktlarergruppe.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            KontaktlarergruppeResource kontaktlarergruppe = objectMapper.convertValue(response.getData().get(0), KontaktlarergruppeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(kontaktlarergruppe);
        }    
    }
