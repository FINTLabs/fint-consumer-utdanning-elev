package no.novari.fint.consumer.models.kontaktlarergruppemedlemskap;
...
public class KontaktlarergruppemedlemskapController {
    ...
    public KontaktlarergruppemedlemskapResources getKontaktlarergruppemedlemskap(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getKontaktlarergruppemedlemskapByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Kontaktlarergruppemedlemskap cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<KontaktlarergruppemedlemskapResource> resources;
    ...
    @PostMapping("/$query")
    public KontaktlarergruppemedlemskapResources getKontaktlarergruppemedlemskapByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getKontaktlarergruppemedlemskap(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private KontaktlarergruppemedlemskapResources getKontaktlarergruppemedlemskapByOdataFilter(
        ...
        Event event = new Event(
                ...
                ElevActions.GET_KONTAKTLARERGRUPPEMEDLEMSKAP, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new KontaktlarergruppemedlemskapResources();
    
        ArrayList<KontaktlarergruppemedlemskapResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<KontaktlarergruppemedlemskapResource>>() {});
