package no.novari.fint.consumer.models.elevforhold;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Elevforhold", value = RestEndpoints.ELEVFORHOLD, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class ElevforholdController {
    ...
    public ElevforholdResources getElevforhold(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getElevforholdByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Elevforhold cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<ElevforholdResource> resources;
    ...
    @PostMapping("/$query")
    public ElevforholdResources getElevforholdByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getElevforhold(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ElevforholdResources getElevforholdByOdataFilter(
        ...
        Event event = new Event(
                ...
                ElevActions.GET_ELEVFORHOLD, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ElevforholdResources();
    
        ArrayList<ElevforholdResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ElevforholdResource>>() {});
    ...
    public ElevforholdResource getElevforholdBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<ElevforholdResource> elevforhold = cacheService.getElevforholdBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return elevforhold.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            ElevforholdResource elevforhold = objectMapper.convertValue(response.getData().get(0), ElevforholdResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(elevforhold);
        }    
    }
