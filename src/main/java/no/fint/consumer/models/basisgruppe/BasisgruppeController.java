...
@Slf4j
@Api(tags = {"Basisgruppe"})
@CrossOrigin
@RestController
@RequestMapping(name = "Basisgruppe", value = RestEndpoints.BASISGRUPPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class BasisgruppeController {
    ...
    @Autowired
    private BasisgruppeLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Basisgruppe cache is disabled.");
        }
    ...
    @GetMapping
    public BasisgruppeResources getBasisgruppe(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getBasisgruppeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Basisgruppe cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<BasisgruppeResource> resources;
    ...
    @PostMapping("/$query")
    public BasisgruppeResources getBasisgruppeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getBasisgruppe(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private BasisgruppeResources getBasisgruppeByOdataFilter(
        ...
        Event event = new Event(
                ...
                ElevActions.GET_BASISGRUPPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new BasisgruppeResources();
    
        ArrayList<BasisgruppeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<BasisgruppeResource>>() {});
    ...
    public BasisgruppeResource getBasisgruppeBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<BasisgruppeResource> basisgruppe = cacheService.getBasisgruppeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return basisgruppe.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            BasisgruppeResource basisgruppe = objectMapper.convertValue(response.getData().get(0), BasisgruppeResource.class);
