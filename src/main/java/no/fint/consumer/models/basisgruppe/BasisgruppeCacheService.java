...
@Slf4j
@Service
@ConditionalOnProperty(name = "fint.consumer.cache.disabled.basisgruppe", havingValue = "false", matchIfMissing = true)
public class BasisgruppeCacheService extends CacheService<BasisgruppeResource> {
    ...
    @Scheduled(initialDelayString = Constants.CACHE_INITIALDELAY_BASISGRUPPE, fixedRateString = Constants.CACHE_FIXEDRATE_BASISGRUPPE)
    ...
    public void onAction(Event event) {
        List<BasisgruppeResource> data;
        if (checkFintResourceCompatibility && fintResourceCompatibility.isFintResourceData(event.getData())) {
            log.info("Compatibility: Converting FintResource<BasisgruppeResource> to BasisgruppeResource ...");
            data = fintResourceCompatibility.convertResourceData(event.getData(), BasisgruppeResource.class);
        } else {
