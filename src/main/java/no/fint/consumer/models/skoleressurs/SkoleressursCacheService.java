...
@Slf4j
@Service
public class SkoleressursCacheService extends CacheService<FintResource<Skoleressurs>> {
    ...
    public Optional<FintResource<Skoleressurs>> getSkoleressursByFeidenavn(String orgId, String feidenavn) {
        return getOne(orgId, (fintResource) -> Optional
                .ofNullable(fintResource)
                .map(FintResource::getResource)
                .map(Skoleressurs::getFeidenavn)
                .map(Identifikator::getIdentifikatorverdi)
                .map(id -> id.equals(feidenavn))
                .orElse(false));
    }

    public Optional<FintResource<Skoleressurs>> getSkoleressursBySystemId(String orgId, String systemId) {
        return getOne(orgId, (fintResource) -> Optional
                .ofNullable(fintResource)
                .map(FintResource::getResource)
                .map(Skoleressurs::getSystemId)
                .map(Identifikator::getIdentifikatorverdi)
    ...
    public void onAction(Event event) {
        update(event, new TypeReference<List<FintResource<Skoleressurs>>>() {
        });
    }
}
