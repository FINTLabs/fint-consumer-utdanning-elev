...
@Slf4j
@Service
public class ElevforholdCacheService extends CacheService<FintResource<Elevforhold>> {
    ...
    @Scheduled(initialDelayString = ConsumerProps.CACHE_INITIALDELAY_ELEVFORHOLD, fixedRateString = ConsumerProps.CACHE_FIXEDRATE_ELEVFORHOLD)
    ...
    public Optional<FintResource<Elevforhold>> getElevforholdBySystemId(String orgId, String systemId) {
        return getOne(orgId, (fintResource) -> Optional
                .ofNullable(fintResource)
                .map(FintResource::getResource)
                .map(Elevforhold::getSystemId)
    ...
    public void onAction(Event event) {
        update(event, new TypeReference<List<FintResource<Elevforhold>>>() {
        });
    }
}
