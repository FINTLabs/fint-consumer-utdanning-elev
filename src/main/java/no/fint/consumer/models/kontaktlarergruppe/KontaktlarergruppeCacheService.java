...
public class KontaktlarergruppeCacheService extends CacheService<KontaktlarergruppeResource> {
    ...
    @Scheduled(initialDelayString = Constants.CACHE_INITIALDELAY_KONTAKTLARERGRUPPE, fixedRateString = Constants.CACHE_FIXEDRATE_KONTAKTLARERGRUPPE)
    ...
    public void onAction(Event event) {
        ...
        if (ElevActions.valueOf(event.getAction()) == ElevActions.UPDATE_KONTAKTLARERGRUPPE) {
            if (event.getResponseStatus() == ResponseStatus.ACCEPTED || event.getResponseStatus() == ResponseStatus.CONFLICT) {
                List<CacheObject<KontaktlarergruppeResource>> cacheObjects = data
                    .stream()
